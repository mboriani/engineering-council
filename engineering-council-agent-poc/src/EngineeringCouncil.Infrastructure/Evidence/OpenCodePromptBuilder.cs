...
/// <summary>
/// Builds the single focused, read-only analysis instruction handed to the OpenCode
/// agentic runtime (Milestone 013.2). It reuses the existing LLM discipline
/// instructions and the existing structured <c>observations</c> output contract —
/// there is deliberately NO OpenCode-specific finding/observation format. The agent
...
/// Engineering Review).
///
/// IMPORTANT: this is an INSTRUCTION-LEVEL read-only boundary, not a security
/// sandbox. OS-level sandboxing/containers remain explicitly deferred; a later
/// milestone must enforce the read-only guarantee outside the prompt.
/// </summary>
public static class OpenCodePromptBuilder
{
