...
/// <summary>
/// Launches the Codex executable against the analyzed repository root. Safety
/// properties (Milestone 013.4), matching the OpenCode runner (M13.2/M13.3):
///
...
///   <see cref="CodexProcessResult.TimedOut"/>.
...
public sealed class CodexProcessRunner : ICodexProcessRunner
{
    ...
    private static Process Start(CodexProcessRequest request)
    {
        // Resolve the configured name to a directly launchable executable. When Codex is
        // installed via npm on Windows only a .cmd/.ps1 shim is on PATH (no codex.exe); the
        // resolver unwraps the shim to its Node script, launched as `node <script> …`, so the
        // child is still started with UseShellExecute=false + ArgumentList and never via a
        // cmd /c string (M14.1).
        ...
        catch (Win32Exception ex)
        {
            throw new LlmProviderException(LlmErrorCategory.ProcessError,
                $"Codex could not be started ('{request.Executable}' in '{request.WorkingDirectory}'). "
