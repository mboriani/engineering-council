...
/// <summary>
/// Milestone 013.2 — focused REAL process tests for <see cref="OpenCodeProcessRunner"/>.
/// They need no OpenCode installation: they launch the dotnet CLI (always present for
/// the build) and, on Windows only, <c>ping</c> to prove timeout/cancellation actually
/// terminate a long-running child. Everything else in the suite uses a fake runner.
///
/// These classes share a NON-PARALLEL collection with the Codex runner tests:
...
public sealed class OpenCodeProcessRunnerTests : IDisposable
{
    private readonly string _workingDir = Path.Combine(Path.GetTempPath(), "ec-opencode-cwd-" + Guid.NewGuid().ToString("N"));

    public OpenCodeProcessRunnerTests()
    {
    ...
    [Fact]
    public async Task OpenCodeProcessRunner_captures_stdout_and_exit_code()
    {
    ...
    [Fact]
    public async Task OpenCodeProcessRunner_enforces_timeout_and_terminates_the_process()
    {
    ...
    [Fact]
    public async Task OpenCodeProcessRunner_propagates_cancellation_and_terminates_the_process()
    {
        if (!OperatingSystem.IsWindows()) return; // ping is a Windows fixture
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => runner.RunAsync(new OpenCodeProcessRequest
        {
            Executable = "ping",
            WorkingDirectory = _workingDir,
            Arguments = ["127.0.0.1", "-n", "30"],   // would run ~30s if never terminated
    ...
    [Fact]
    public async Task OpenCodeProcessRunner_timeout_terminates_the_complete_process_tree()
    {
        // Real-runtime compatibility (M13.3A): a timeout must terminate the WHOLE process
        // tree, not just the directly-started process. `cmd /c ping -n 30` runs ~30s and
        // spawns `ping` as a child — if only the parent were killed, the ping grandchild
        // would survive as an orphan. Snapshot existing ping PIDs so we only count new ones.
    ...
    [Fact]
    public async Task OpenCodeProcessRunner_cancellation_terminates_the_complete_process_tree()
    {
        // Real-runtime compatibility (M13.3A): run cancellation must terminate the whole
        // process tree too (parent `cmd` plus its child `ping`), never leaving orphans.
        if (!OperatingSystem.IsWindows()) return; // cmd/ping are Windows fixtures
